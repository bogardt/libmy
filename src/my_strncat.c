 ...
 * @param dest string If dest is not large enough, program behavior is
 ...
 programs.
 * @param src string If src contains n or more bytes, strncat() writes n+1 bytes
 to dest (n from src plus the terminating null byte). Therefore, the size of
 ...
 * @note The my_strncat() is similar to the strcat() function, except that
 ...
 bytes. If src contains n or more bytes, strncat() writes n+1 bytes to dest (n
 from src plus the terminating null byte).  Therefore, the size of dest must be
